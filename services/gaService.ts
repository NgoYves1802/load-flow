...
function calculateAverageFitness(fitnessScores: number[]): number {
  return fitnessScores.reduce((sum, f) => sum + f, 0) / fitnessScores.length;
}

function calculateFitnessDiversity(fitnessScores: number[]): { stdDev: number, range: number } {
  const mean = calculateAverageFitness(fitnessScores);
  const variance = fitnessScores.reduce((sum, f) => sum + Math.pow(f - mean, 2), 0) / fitnessScores.length;
  const stdDev = Math.sqrt(variance);
  const range = Math.max(...fitnessScores) - Math.min(...fitnessScores);
  return { stdDev, range };
}

function calculateGenotypicDiversity(population: number[][]): number {
  ...
  for (let i = 0; i < population.length; i++) {
    for (let j = i + 1; j < population.length; j++) {
      const distance = Math.sqrt(
        population[i].reduce((sum, val, idx) => sum + Math.pow(val - population[j][idx], 2), 0)
      );
...
function logConvergenceMetrics(generation: number, fitnessScores: number[], population: number[][], bestFitness: number): void {
  const avgFitness = calculateAverageFitness(fitnessScores);
  const { stdDev, range } = calculateFitnessDiversity(fitnessScores);
  const genotypicDiversity = calculateGenotypicDiversity(population);

  console.log(`=== Generation ${generation} Convergence Metrics ===`);
  console.log(`Best Fitness: ${bestFitness.toExponential(4)}`);
  console.log(`Average Fitness: ${avgFitness.toExponential(4)}`);
  console.log(`Fitness Std Dev: ${stdDev.toExponential(4)}`);
  console.log(`Fitness Range: ${range.toExponential(4)}`);
  console.log(`Genotypic Diversity: ${genotypicDiversity.toFixed(6)}`);
...
function mutate(chromosome: number[], rate: number, network: NetworkState, vMin: number, vMax: number): number[] {
  ...
  for (let i = 0; i < chromosome.length; i++) {
    if (Math.random() < rate) {
      if (i < n) {
        // Voltage mutation - ONLY for PQ buses
        if (network.nodes[i].type === NodeType.PQ) {  // ✅ FIXED!
          mutated[i] += (Math.random() - 0.5) * 0.1;
          mutated[i] = Math.max(vMin, Math.min(vMax, mutated[i]));
        }
        // PV and SLACK buses: voltage stays fixed (no mutation)
      } else {
        // Angle mutation - for PV and PQ buses (NOT slack)
        const busIdx = i - n;
...
export async function calculateLoadFlowGA(
  ...
  for (let i = 0; i < populationSize; i++) {
    ...
    // Voltage magnitudes
for (let j = 0; j < n; j++) {
  if (network.nodes[j].type === NodeType.SLACK || 
      network.nodes[j].type === NodeType.PV) {
    // PV and SLACK: use specified voltage
    chromosome.push(network.nodes[j].voltage);
  } else {
    // PQ: initialize to flat start (1.0 p.u.)
    chromosome.push(1.0);
  }
}
  ...
  const initialFitnessScores = population.map(ind => fitness(ind, network, Ybus, minVoltage, maxVoltage, baseMVA, alpha, beta, gamma));
  logConvergenceMetrics(0, initialFitnessScores, population, Math.min(...initialFitnessScores));
  console.log(`Starting GA optimization with ${populationSize} individuals, max ${maxGenerations} generations\n`);
  ...
  for (let gen = 0; gen < maxGenerations; gen++) {
    ...
    // Log convergence metrics every 10 generations or at key points
    if (gen % 10 === 0 || gen < 5 || minFitness < bestFitness * 1.1) {
      logConvergenceMetrics(gen + 1, fitnessScores, population, bestFitness);
    }
    ...
    while (newPop.length < populationSize) {
      ...
      // Crossover
      // After crossover, restore PV and SLACK voltages
if (Math.random() < crossoverRate) {
  const point = Math.floor(Math.random() * parent1.length);
  child1 = [...parent1.slice(0, point), ...parent2.slice(point)];
  child2 = [...parent2.slice(0, point), ...parent1.slice(point)];
  
  // ✅ FIX: Restore fixed voltages
  for (let i = 0; i < n; i++) {
    if (network.nodes[i].type === NodeType.SLACK || 
        network.nodes[i].type === NodeType.PV) {
      child1[i] = network.nodes[i].voltage;
      child2[i] = network.nodes[i].voltage;
    }
  }
}
...
child2 = mutate(child2, adaptiveMutRate, network, minVoltage, maxVoltage);
  ...
  console.log(`Final Best Fitness: ${bestFitness.toExponential(4)}`);
  console.log(`Convergence Threshold: ${convergenceThreshold.toExponential(4)}`);
  console.log(`Converged: ${bestFitness < convergenceThreshold ? 'YES' : 'NO'}`);
  console.log(`Total Generations: ${fitnessHistory.length}`);
  console.log(`Network Size: ${n} buses, ${network.edges.length} lines`);
